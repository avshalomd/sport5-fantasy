...
module$dist$src$ads$logging.recordEvent=recordEvent$$module$dist$src$ads$logging;module$dist$src$ads$logging.reportError=reportError$$module$dist$src$ads$logging;/*
 (c)Lalit Patel, MODIFIED apache.org/licenses/LICENSE-2.0*/
function createScript$$module$dist$src$shared$dom(a){var b=document.createElement("script");b.type="text/javascript";b.src=a;b.onerror=function(b){reportError$$module$dist$src$ads$logging("Error loading script: "+a,b)};return b}function docWrite$$module$dist$src$shared$dom(a){try{var b=(new DOMParser).parseFromString(a,"text/html");document.currentScript.parentNode.insertBefore(b,document.currentScript)}catch(c){reportError$$module$dist$src$ads$logging("docWrite Error",c)}}
...
status:getStatusCodes$$module$dist$src$impressionContainer$logToServer().join()};a.elapsed_ms=b.elapsed_ms=(new Date).getTime()-a.start_time;(new Image).src=c+"?"+buildQS$$module$dist$src$shared$url(b);log$$module$dist$src$impressionContainer$logging("Logged Container Tag Successfully")}var module$dist$src$impressionContainer$logToServer={};module$dist$src$impressionContainer$logToServer.logToServer=logToServer$$module$dist$src$impressionContainer$logToServer;function fireTag$$module$dist$src$impressionContainer$tags(a){function b(a,b){log$$module$dist$src$impressionContainer$logging("ERROR FIRING TAG "+a.id);$jscompDefaultExport$$module$dist$src$impressionContainer$config.setStatusCode(a.id,"js_error");reportError$$module$dist$src$impressionContainer$logging("Tag "+a.id+": "+a.error.replace(/["']/g,""),b);return!1}var c=$jscompDefaultExport$$module$dist$src$impressionContainer$config.getConfig();switch(a.type){case "img":try{(new Image).src=a.src,c.img_tags_fired++}catch(d){return b(a,
d)}break;case "script":try{if(!1===eval(a.src))return $jscompDefaultExport$$module$dist$src$impressionContainer$config.setStatusCode(a.id,"custom_js_logic_failed"),!1;c.script_tags_fired++}catch(d){return b(a,d)}}$jscompDefaultExport$$module$dist$src$impressionContainer$config.ctConfig.globals.fired_tags[a.id]=a.id;c.total_tags_fired++;$jscompDefaultExport$$module$dist$src$impressionContainer$config.setStatusCode(a.id,"fired");c.tss.push((new Date).getTime()-c.start_time);return!0}
